...
//  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = ???
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.map{case (qid: QID, it: Iterable[(Question, Answer)]) => (
      it.map(_._1).head, answerHighScore(it.map(_._2).toArray)
    )}
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map{
      case (question: Question, highScore: HighScore) => (firstLangInTag(question.tags, langs).getOrElse(-1) * langSpread, highScore)
    }.filter(_._1 >= 0).cache()
  }
...
//    val newMeans = cluster.reduceByKey((x,y) => (x + y))
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langsGroupedScore = vs.groupBy(_._1).mapValues(_.size)
      val mostCommonLang = langsGroupedScore.maxBy(_._2)
      val langsScore = langsGroupedScore.values.toArray.sorted

      // most common language in the cluster
      val langLabel: String   = langs(mostCommonLang._1/langSpread)
      // percent of the questions in the most common language
      val langPercent: Double = (mostCommonLang._2 / vs.size.toDouble) * 100
